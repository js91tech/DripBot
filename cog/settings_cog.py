...
class SettingsCog(commands.Cog):
    ...
    @group.command(name="mode", description="Switch between Markov and LLM brain mode")
    ...
    async def roast(self, interaction: discord.Interaction, user: discord.Member):
        ...
        response = await generate_llm_response(roast_prompt, chat_history, model_name=model_name)
        if response:
    ...
    async def reset_data(self, interaction: discord.Interaction):
        ...
        # FIX: None check before accessing .chains
        chat_cog = self.bot.get_cog("Chat")
        if chat_cog and interaction.guild.id in chat_cog.chains:
            del chat_cog.chains[interaction.guild.id]
