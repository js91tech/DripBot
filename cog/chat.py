...
class Chat(commands.Cog):
        ...
        for guild in self.bot.guilds:
            ...
            async for msg in target_channel.history(limit=50):
                ...
                if msg.author == self.bot.user:
                    ...
                    content_text = clean_msg_content if clean_msg_content else "sent an image"
                    chat_history.insert(
                        0, {"role": "user", "content": f"{display_name}: {content_text}"})
            ...
            response = await generate_llm_response(prompt, chat_history, model_name=model_name)
    ...
    async def memory_consolidation_loop(self):
        ...
        for guild in self.bot.guilds:
            ...
            model_name = settings.get("llm_model", "meta-llama/llama-3-8b-instruct")
            summary = await generate_llm_response(prompt, chat_history, model_name=model_name)
            if summary:
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Bots and commands don't trigger responses
        ...
        if should_respond:
            ...
            async with message.channel.typing():
                ...
                # FIX: Only use brain_mode to decide, not model name substring matching
                use_llm = settings.get("brain_mode") == "llm" and not use_gif
                ...
                if use_llm:
                    ...
                    async for msg in message.channel.history(limit=100):
                        ...
                        # FIX: Skip empty messages with no attachments before building payload
                        ...
                        if msg.author == self.bot.user:
                            ...
                            # FIX: Use variable to avoid nested-quote f-string SyntaxError
                            display_name = msg.author.display_name
                            content_text = clean_msg_content if clean_msg_content else "sent an image"
                            text_part = f"{display_name}: {content_text}"

                            # Only use multimodal format if there are image attachments
                            has_images = any(
                                att.content_type and "image" in att.content_type
                                for att in msg.attachments
                            )
                            if has_images:
                                content_payload = [{"type": "text", "text": text_part}]
                                for att in msg.attachments:
                                ...
                                chat_history.insert(0, {"role": "user", "content": text_part})

                    # Insert the trigger message at the end
                    clean_trigger_content = re.sub(r'<@!?\d+>', '', message.content).strip()
                    trigger_display_name = message.author.display_name
                    trigger_text = clean_trigger_content if clean_trigger_content else "sent an image"
                    trigger_text_part = f"{trigger_display_name}: {trigger_text}"

                    has_trigger_images = any(
                        att.content_type and "image" in att.content_type
                        for att in message.attachments
                    )
                    if has_trigger_images:
                        trigger_payload = [{"type": "text", "text": trigger_text_part}]
                        for att in message.attachments:
                            if att.content_type and "image" in att.content_type:
                        ...
                        chat_history.append({"role": "user", "content": trigger_text_part})
                    ...
                    if user_memories:
                        ...
                        dynamic_prompt += (
                            f"\n\nPermanent memories about {display_name}:\n"
                            f"{memory_str}\nBe a smart-ass about these."
                        )

                    model_name = settings.get("llm_model", "meta-llama/llama-3-8b-instruct")
                    llm_response = await generate_llm_response(
                        dynamic_prompt, chat_history, model_name=model_name
                    )
                    if llm_response:
