...
class Database:
    ...
    async def init(self):
        ...
        # FIX: Added PRIMARY KEY (guild_id, user_id, rowid) to prevent duplicates
    ...
    async def get_all_guild_stats(self):
        """Get stats for all guilds (used by dashboard)."""
        cursor = await self.conn.execute(
            "SELECT guild_id, messages_learned, messages_sent FROM stats"
        )
        rows = await cursor.fetchall()
        return [
            {"guild_id": row[0], "messages_learned": row[1], "messages_sent": row[2]}
            for row in rows
        ]
